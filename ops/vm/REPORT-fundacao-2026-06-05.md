...
1. **Firewall** — na VM: `bash /opt/ifp-connect/ops/vm/harden-firewall.sh` (libera SSH **antes** de ativar; seguro). _Precisa do arquivo na VM — entra no próximo deploy, ou copio na hora._
...
4. **Deploy** (pra os headers §3 + qualquer commit valerem em produção):
   ...
   ```
   _(Eu posso fazer isso com você acordado — deploy não é autônomo.)_
