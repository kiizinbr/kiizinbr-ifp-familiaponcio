...
## 12. SITE INSTITUCIONAL PÚBLICO (one-page) — adicionado depois do kit

Porta de entrada pública do IFP Connect. Tom **energético/vibrante**, fé com lugar de honra,
**sem apelo a doação** e **sem parcerias institucionais** (o projeto é 100% da Família Pôncio,
não depende de outras instituições).

**Arquivos:**
```
Site Institucional.html   ← a página (estrutura + conteúdo real)
site.css                  ← camada de apresentação do site público (apoia-se em ifp-tokens.css)
site.js                   ← splash, hero slideshow, dropdown de acesso, Palavra do Dia, reveals
image-slot.js             ← componente de imagem arrastável (banner aéreo)
fotos/aerea-centro-medico.jpg     ← foto de drone (Centro Médico)
fotos/aerea-capacitacao.jpg       ← foto de drone (Capacitação)
loaders/lion-white.png            ← leão branco recortado (medalhões/hero)
```
> Fotos das atividades vêm do CDN do site atual (static.wixstatic.com) referenciadas por URL.
> Para produção, baixe-as e sirva localmente.

**Seções (em ordem):** Hero → Unidades (4 cards informativos) → Banner aéreo "Nossa casa"
(image-slot, troca a foto de drone arrastando) → Quem somos (+ foto da equipe) → Momentos
(galeria) → Impacto (KPIs count-up) → **Faça parte** (voluntariado/participe, NÃO parcerias) →
Rodapé/contato. O versículo NÃO é mais uma seção — virou o popup "Palavra do Dia".

**Hero — slideshow institucional sincronizado:** o fundo cruza entre as fotos das 4 frentes
(2 aéreas de drone + jiu-jitsu + crianças). A cada troca, o **medalhão-leão da unidade
correspondente acende** (escala + brilho) e os outros esmaecem; o rótulo "Em destaque: …" e o
filtro de cor mudam junto. 4 pontinhos (`.hn-dots`) deixam pular/escolher. Auto a cada 4,4s,
pausa no hover. Lógica em `site.js`, bloco "HERO: slideshow institucional".

**Header + palavras do hero em degradê:** o nome "Instituto Família Pôncio" (`.brand-grad`) e as
palavras **saúde/educação/amor** (`.hl-saude/.hl-edu/.hl-amor`) usam
`background-image: linear-gradient(...); -webkit-background-clip:text; -webkit-text-fill-color:transparent;`
com cores das unidades. ⚠️ Texto com `background-clip:text` costuma renderizar **cortado em
screenshots/html-to-image**, mas no navegador real fica completo — confira no browser, não em print.

**"Acesso ao Sistema" (único acesso, no topo):** botão na nav abre dropdown com as 4 unidades +
grupo "equipe interna" (Serviço Social, Executivo). Cada item navega ao login da unidade com uma
transição de leão (overlay). Os cards de unidade são **só informativos** (não logam).

**Logins split-screen** (`preview/login-medico.html`, `preview/login-capacitacao.html`,
`scaffolds/login-tematico.html`): duas colunas — foto da unidade (com overlay da cor) à esquerda,
formulário à direita; empilham no mobile. O temático lê `location.hash` (`#esportivo`, `#recreativo`,
`#social`, `#poncio`) para abrir já na unidade certa, e tem um switcher que troca foto+cor+nome.

**Palavra do Dia (popup de boas-vindas):** abre ~0,45s depois do splash, **1x por dia** por
visitante (`localStorage` chave `ifp-daily-AAAA-MM-DD`, data LOCAL). 14 reflexões giram por
dia-do-ano (`day % 14`). Fecha por "Amém", X, clique no fundo ou Esc. Para mudar a frequência:
trocar o gate de `localStorage` por `sessionStorage` (1x por sessão) ou remover (toda visita).
Reflexões e lógica no bloco "PALAVRA DO DIA" de `site.js`.

**Notas de implementação no Next.js:** `Site Institucional.html` é a landing pública (rota `/`).
`site.css`/`site.js` viram um módulo da landing; os blocos JS (slideshow, popup, dropdown) viram
hooks/efeitos React. O popup pode ser um client component com o mesmo gate de localStorage. As
fotos de drone devem ir para `/public`. O acesso ao sistema (dropdown) aponta para as rotas de
login por unidade (que já existem no app autenticado do kit).

---
_Restrições de marca: paleta do brandbook e logos são canônicos — muda só o uso. Direção: clínica premium (não-Editorial) no app; calorosa/vibrante no site público._
