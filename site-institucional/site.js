...
(function () {
  ...
  document.querySelectorAll("[data-open-access]").forEach(function (el) {
    el.addEventListener("click", function (e) { e.preventDefault(); window.scrollTo({ top: 0, behavior: reduce ? "auto" : "smooth" }); openAccess(); });
  });
  ...
  document.querySelectorAll("[data-go]").forEach(function (el) {
    el.addEventListener("click", function (e) {
      ...
      goWithLion(el.getAttribute("data-go"), el.getAttribute("data-unit"), el.getAttribute("data-phrase"), ox, oy);
    });
  });
  ...
  function animateCount(el) {
    ...
    if (reduce) { el.textContent = prefix + target.toLocaleString("pt-BR"); return; }
  ...
  (function () {
    ...
    var names = { medico: "Centro Médico", capacitacao: "Centro de Capacitação", esportivo: "Centro Esportivo", recreativo: "Centro Recreativo" };
    var colors = { medico: "#007571", capacitacao: "#ff772e", esportivo: "#c24d0f", recreativo: "#10c2bb" };
    var i = 0, timer = null;
    function show(n) {
    ...
    function start() { stop(); if (!reduce) timer = setInterval(next, 4400); }
  ...
  (function () {
    ...
    document.getElementById("dailyDate").textContent = now.getDate() + " de " + meses[now.getMonth()] + " de " + now.getFullYear();

    var key = "ifp-daily-" + now.getFullYear() + "-" + ("0" + (now.getMonth() + 1)).slice(-2) + "-" + ("0" + now.getDate()).slice(-2);
    function open() {
    ...
    function trigger() { setTimeout(open, 450); }
